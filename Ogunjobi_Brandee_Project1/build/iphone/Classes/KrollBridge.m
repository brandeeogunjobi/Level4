...
NSString * Ogunjobi_Brandee_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Ogunjobi_Brandee_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ogunjobi_brandee_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Ogunjobi_Brandee_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Ogunjobi_Brandee_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ogunjobi_brandee_project1 = [[Ogunjobi_Brandee_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ogunjobi_brandee_project1];
	
	NSString *_ogunjobi_brandee_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ogunjobi_brandee_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ogunjobi_brandee_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Ogunjobi_Brandee_Project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.ogunjobi_brandee_project1.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.ogunjobi_brandee_project1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
