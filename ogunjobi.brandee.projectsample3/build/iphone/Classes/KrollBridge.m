...
NSString * ogunjobibrandeeprojectsample3$ModuleRequireFormat = @"(function(exports){"
...
@implementation ogunjobibrandeeprojectsample3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ogunjobibrandeeprojectsample3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_ogunjobibrandeeprojectsample3 gc];
}
...
{
	// create ogunjobibrandeeprojectsample3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ogunjobibrandeeprojectsample3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ogunjobibrandeeprojectsample3 = [[ogunjobibrandeeprojectsample3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ogunjobibrandeeprojectsample3];
	
	NSString *_ogunjobibrandeeprojectsample3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ogunjobibrandeeprojectsample3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ogunjobibrandeeprojectsample3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:ogunjobibrandeeprojectsample3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.ogunjobibrandeeprojectsample3.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.ogunjobibrandeeprojectsample3.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
